...
const AdminDepartamentos = () => {
  ...
  const guardar = async () => {
    ...
    try {
      if (seleccionado) await api.put(`/departamentos/${seleccionado.id}`, form);
      else await api.post('/departamentos', form);
      setModal(false);
      cargarDepartamentos();
      alert(seleccionado ? 'Departamento actualizado' : 'Departamento creado');
    } catch (error) { alert(error.response?.data?.mensaje || 'Error al guardar'); }
  };

  const eliminar = async (item) => {
    if (!window.confirm(`¿Eliminar "${item.nombre}"?`)) return;
    try { await api.delete(`/departamentos/${item.id}`); cargarDepartamentos(); alert('Departamento eliminado'); }
    catch (error) { alert(error.response?.data?.mensaje || 'No se pudo eliminar'); }
  };

  if (cargando) return <div className="flex justify-center py-20 text-principal">Cargando departamentos...</div>;

  return (
    <div className="py-4">
      <h1 className="text-2xl font-bold text-principal mb-1">Departamentos</h1>
      <p className="text-gray-400 text-sm mb-4">Gestiona los departamentos de la empresa</p>
      ...
      <div className="space-y-3">
        {departamentos.map(item => (
          <div key={item.id} className="bg-white rounded-2xl p-4 shadow-sm border-l-4 border-acento">
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm">
            <h2 className="text-lg font-bold text-principal mb-4">{seleccionado ? 'Editar departamento' : 'Nuevo departamento'}</h2>
            ...
            <input className="w-full border border-gray-200 rounded-xl p-3 bg-gray-50 text-principal mb-3 focus:outline-none"
              placeholder="Ej. PRODUCCION, CALIDAD..." value={form.nombre}
            ...
            <textarea className="w-full border border-gray-200 rounded-xl p-3 bg-gray-50 text-principal mb-4 focus:outline-none resize-none"
              rows={3} placeholder="Describe el departamento..." value={form.descripcion}
            ...
            <button onClick={guardar}
              ...
              {seleccionado ? 'Guardar cambios' : 'Crear departamento'}
            </button>
